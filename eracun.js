...
streznik.get('/', function(zahteva, odgovor) {
  ...
  if(zahteva.session.userId == undefined){

    var tmp = zahteva.headers.host;
    odgovor.redirect("https://" +tmp +  "/prijava");
  }
  else{
...
streznik.post('/izpisiRacunBaza', function(zahteva, odgovor) {
  var tmp = new formidable.IncomingForm();
  tmp.parse(zahteva,function(napaka1,polja,datoteke){
    var racID = polja.seznamRacunov;
    pesmiIzRacuna(racID,function(pesmi){
      strankaIzRacuna(racID,function(stranka){
...
streznik.get('/izpisiRacun/:oblika', function(zahteva, odgovor) {
  pesmiIzKosarice(zahteva, function(pesmi) {
    if (!pesmi) {
      ...
      userId(zahteva.session.userId, function(napaka, vrstice){
         if(napaka){
           odgovor.end();
         } 
         else{
...
//vrni userId
var userId = function(userId, callback){
...
streznik.post('/odjava', function(zahteva, odgovor) {
    zahteva.session.userId = undefined;
    odgovor.redirect('/prijava') 
})
